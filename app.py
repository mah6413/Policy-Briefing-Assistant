...
st.caption("Generate policy answers, memos, briefs, letters, and executive summaries from uploaded documents.")
...
with st.sidebar:
    ...
    if demo_mode:
        ...
        st.info("The app will return realistic sample outputs without calling the OpenAI API.")
    else:
        if os.getenv("OPENAI_API_KEY"):
            st.success("Live AI Mode is ON")
        else:
            st.error("OPENAI_API_KEY not found. Add it to your environment or switch Demo Mode back on.")
    ...
    st.markdown("---")
    st.markdown("**Tip:** Keep Demo Mode on for public sharing so your app does not burn tokens.")

st.info(
    "This public demo can run in a no-token mode for safe sharing. "
    "Live AI generation is available when Demo Mode is turned off and an API key is configured."
)
...
user_query = st.text_area(
    "Enter your prompt",
    placeholder="Example: Summarize the main policy recommendations from these documents.",
    height=140,
)
    ...
    try:
        ...
        if sources:
            ...
            for i, source in enumerate(sources, start=1):
                with st.expander(f"Source {i}: {source.get('document_title', 'Untitled')}"):
